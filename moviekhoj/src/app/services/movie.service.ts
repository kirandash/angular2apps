...
export class MovieService {
  ...
  getMovie(id:string) {
    // Callback and API keys are important
    return this._jsonp.get('https://api.themoviedb.org/3/movie/'+id+'?callback=JSONP_CALLBACK&api_key='+this.apikey).map(res => res.json()); // map is used since it is going to be an observable
  }
}
